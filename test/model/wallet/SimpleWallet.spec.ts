...
describe('SimpleWallet', () => {
    it('should createComplete a new simple wallet', () => {
        const simpleWallet = SimpleWallet.create('wallet-name', new Password('password'), NetworkType.PUBLIC_TEST_NET);
        expect(simpleWallet.name).to.be.equal('wallet-name');
        expect(simpleWallet.network).to.be.equal(NetworkType.PUBLIC_TEST_NET);
    ...
    it('should createComplete a new wallet with privateKey', () => {
        ...
        const account = Account.createFromPrivateKey(privateKey, NetworkType.PUBLIC_TEST_NET);
        const simpleWallet = SimpleWallet.createFromPrivateKey('wallet-name', new Password('password'), privateKey, NetworkType.PUBLIC_TEST_NET);
        expect(simpleWallet.name).to.be.equal('wallet-name');
        expect(simpleWallet.network).to.be.equal(NetworkType.PUBLIC_TEST_NET);
